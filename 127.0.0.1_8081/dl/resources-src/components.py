...
def L(up, target):
    """Internal link helper. Absolute site paths are used as-is so Resources
    pages share the current site's clean URLs (/work, /free-audit, …)."""
    if target.startswith("/") or target.startswith("http"):
        return target
    return up + target


# Canonical internal destinations — current site clean URLs.
WORK = "/work"
RESEARCH = "/research"
INSIGHTS = "/insights"
ABOUT = "/about"
